...
for /f "usebackq delims=" %%I in (`powershell -NoProfile -Command "(Get-NetIPAddress -AddressFamily IPv4 ^| Where-Object {$_.PrefixOrigin -ne 'WellKnown' -and $_.IPAddress -notlike '169.*'} ^| Select-Object -First 1).IPAddress"`) do set "IP=%%I"
echo.
