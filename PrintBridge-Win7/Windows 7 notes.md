...
| `Printer Quality Settings.bat` | Reads the default printer from the registry rather than `Get-CimInstance`, which needs PowerShell 3. |
| `Stop Starting With Windows.bat` | Stops the hidden process with `wmic` instead of `Get-CimInstance`. |
