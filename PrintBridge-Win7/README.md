# Print Bridge — Windows 7 edition

This is the Windows 7 build. It is the same bridge; only the setup scripts
differ, because Windows 7 has no `winget` and ships PowerShell 2.0. Read
**`Windows 7 notes.md`** first — it lists what you install by hand.
...
- finds Python, and tells you where to get it if it's missing (Windows 7 needs
  3.8.10 — see the notes file)
- installs the one library it needs for network announcements
- checks for **SumatraPDF**, which prints a PDF with no dialog and, more
  importantly, **with rescaling switched off** — that's what makes a page come
  out at its exact physical size — and links you to it if it isn't there
