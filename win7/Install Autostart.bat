...
rem  The package lives one folder up.
cd /d "%~dp0.."
set "ROOT=%CD%"
...
schtasks /Create /TN "Print Bridge" /SC ONLOGON /RL HIGHEST /F ^
  /TR "\"!PYW!\" \"%ROOT%\run_hidden.pyw\"" >nul 2>&1
if errorlevel 1 (
  echo   ! Could not create the scheduled task.
  goto :done
)
echo   Task:     created - it will start at every logon
echo             and sit in the tray, next to the clock
echo.
...
echo       Log file:  %ROOT%\printbridge.log
echo.
