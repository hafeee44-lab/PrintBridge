...
class Job:
    ...
    def __init__(self, jid, queue, name="Untitled", user="anonymous",
        ...
        self.mono = True
        self.duplex = False
        self.duplex_edge = "long"          # or "short", when the client says so
        self.media = ""
    ...
    def attrs(self, requested=None, up_time=0):
        ...
        all_attrs = [
            ...
            keyword("sides", ("two-sided-short-edge"
                              if self.duplex_edge == "short"
                              else "two-sided-long-edge")
                             if self.duplex else "one-sided"),
...
class Queue:
    ...
    def __init__(self, printer_name, backend, host="127.0.0.1", port=631,
                 ...
                 make_and_model=None, brand="", duplex=False):
    ...
    def new_job(self, name, user, fmt):
        with self._lock:
            ...
            if len(self.jobs) > 200:
                # the ones falling off the end take their spool files with
                # them - a finished job has none, but one abandoned halfway
                # would otherwise sit in the spool folder forever
                for old in self.jobs[:-200]:
                    self._cleanup(old)
    ...
    def printer_attrs(self, requested=None):
        ...
        attrs = [
            ...
            keyword("sides-supported", *(
                ...
                if self.duplex_capable else ("one-sided",))),
