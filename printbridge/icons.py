...
def ico(sizes=(16, 32, 48, 256)):
    """The same icon as a Windows .ico, for the tray and the executable.

    An .ico is a small directory followed by the images themselves, and since
    Vista those images are allowed to be PNGs - which is convenient, because a
    PNG writer is what we already have.
    """
    images = [(s, get(s)) for s in sorted(set(sizes))]
    count = len(images)
    header = struct.pack("<HHH", 0, 1, count)          # reserved, type=icon, count
    offset = 6 + 16 * count               # ICONDIR is 6 bytes, each entry 16
    entries, blob = b"", b""
