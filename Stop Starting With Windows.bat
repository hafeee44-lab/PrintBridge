...
powershell -NoProfile -Command "Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like '*run_hidden.pyw*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }" >nul 2>&1
rem  wmic as well, for Windows 7, which has no Get-CimInstance. The [_] stops
rem  the pattern matching this command's own line and killing wmic itself.
